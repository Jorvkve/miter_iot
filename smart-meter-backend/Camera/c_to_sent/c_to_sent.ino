...
String serverUrl = "http://tall-cow-82.loca.lt/api/upload"; 
...
void setup() {
  ...
  if(psramFound()){
    // ✅ เปลี่ยนเป็น QVGA (320x240) เล็กแต่ส่งไวแน่นอน
    config.frame_size = FRAMESIZE_QVGA; 
    config.jpeg_quality = 12;  // ปรับให้ชัดหน่อย (เลขน้อย = ชัด)
    config.fb_count = 1;
  } else {
    config.frame_size = FRAMESIZE_QVGA; // แก้ตรงนี้ด้วย
...
void loop() {
  // เช็คว่ามีการพิมพ์คำสั่งมาใน Serial Monitor หรือไม่
  if (Serial.available()) {
    char cmd = Serial.read();
    
    // ถ้าพิมพ์ 'c' หรือ 'C' ให้เริ่มทำงาน
    if (cmd == 'c' || cmd == 'C') {
      Serial.println("\n--- Command Received: Taking Photo ---");
      
      // 1. เคลียร์ค่าขยะ และต่อเน็ตใหม่ (สูตรเดิมที่เสถียรแล้ว)
      while(sim800.available()) sim800.read();
      initGPRS(); 
      ...
      Serial.println("--- Job Done. Waiting for next command ('c') ---");
    }
  }
  
  // ไม่ต้อง delay นานๆ แล้ว เพราะเราไม่ได้วนลูปอัตโนมัติ
  // ใส่ delay นิดหน่อยเพื่อไม่ให้ CPU ทำงานหนักเกินไปขณะรอรับคำสั่ง
  delay(100); 
}
...
void sendImageToBackend(camera_fb_t * fb) {
  ...
  for (size_t i = 0; i < fb->len; i += chunkSize) {
    ...
    // ✅ [เพิ่มตรงนี้] หยุดพัก 50ms ทุกๆ 1KB เพื่อช่วย SIM800L
    delay(50); 
  }
  ...
  Serial.println("\nPOST Action...");
  // รอ 2 นาที (120000) ถูกต้องแล้วครับ
